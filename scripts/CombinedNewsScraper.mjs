...
scraped_at: new Date().toISOString()  // <-- Changed to snake_case
};
...
processedArticle.relevance_reason = relevance.explanation;  // Changed to snake_case
...
console.log('Debug: sitesToScrape =', sitesToScrape, 'startIndex =', config.startIndex, 'maxSites =', config.maxSites);
console.log('Debug: finalSitesToScrape =', finalSitesToScrape);
...
} else if (args[i] === '--no-ml') {  // NEW: Add this option
...
--no-ml                   : Disable ML model relevance detection (NEW)
...
// Check with ML model first (if not disabled)
if (process.env.DISABLE_ML_MODEL !== 'true') {
try {
...
try {
  ...
  console.log(`Note: Couldn't take button screenshot: ${screenshotErr.// scripts/CombinedNewsScraper.mjs
...
const correctedTitle = response.data.choices[0].message.content.